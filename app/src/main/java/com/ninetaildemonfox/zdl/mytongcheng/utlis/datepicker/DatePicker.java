...
//        addView(rlTitle, llParams);
        ...
        for (int i = 0; i < mLManager.titleWeek().length; i++) {
            ...
            tvWeek.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 10);
    ...
    public void setSign(TextView tv_sign) {
        this.tv_sign = tv_sign;
    }
    ...
    public void setRightTitle(boolean flag) {
        ...
        if (isSignIn) {
            mySignIn.setText("已签到");
            mySignIn.setTextColor(mContext.getResources().getColor(R.color.whileColor));
//            signIn.setBackgroundDrawable(mContext.getResources().getDrawable(R.drawable.));
        } else {
